...
locals {
  additional_iam_members = var.additional_iam_members == [] ? {} : {
    "additional_iam_members" = {
      "roles/compute.networkUser" = var.additional_iam_members
    }
  }
...
resource "google_compute_subnetwork_iam_member" "iam_additional_members" {
  for_each = var.additional_iam_members == [] ? toset([]) : var.additional_iam_members
